...
## 1. 基础信息
- 候选ID：YeastBridge_Mol031
- 靶点名称：HRH1（UniProt P35367，gpcr；ChEMBL CHEMBL231）
- 主要功能：功能性调节（EC50/AC50 测定，p=6.07；激动/拮抗方向未实验判定）
- 化合物：InChIKey QOYHHIBFXOOADH-UHFFFAOYSA-N；ChEMBL CHEMBL46516

## 2. 计算设计方案
- 分子结构（SMILES）：`C1CN(CCC12C(=O)NCN2C3=CC=CC=C3)CCCC(C4=CC=C(C=C4)F)C5=CC=C(C=C5)F`（另见 YeastBridge_Mol031_ligand.smi / _ligand_pose.sdf）
- 小分子-靶点复合物结构预测文件：YeastBridge_Mol031_complex.pdb（蛋白取自 P35367 实验结构的清洁聚合物链；配体为 Vina-GPU 最优口袋 pose）
...
- 设计逻辑：人功能靶点的任务经 scFoundation 酵母表示迁移（B 路线，ESM2 注入）定义酵母可执行任务；化合物在酵母中显著执行任务后，其人靶点身份由定量药理注释确立，再以对接（结合合理性，-7.2 kcal/mol）与双引擎药效学评估确认——DTI 预训练集成（校准后 pIC50 6.76）与同靶点分子近邻法（kNN pIC50 6.28，最近邻 Tanimoto 0.28，即与该靶点已知配体的结构相似度）。实测 pChEMBL 6.07。
