...
- 设计逻辑：人功能靶点的任务经 scFoundation 酵母表示迁移（B 路线，ESM2 注入）定义酵母可执行任务；化合物在酵母中显著执行任务后，其人靶点身份由定量药理注释确立，再以对接（结合合理性，{r.docking_affinity:.1f} kcal/mol）与预训练效价模型（集成 pIC50 {r.pd_pic50_mean:.2f}，实测 pChEMBL {r.pchembl_measured}）双重确认。
