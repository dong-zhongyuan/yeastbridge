#!/usr/bin/env python
"""ESM2 per-protein mean-pooled embedding extractor for arbitrary FASTA
inputs (product step 2 candidate proteins; also reusable elsewhere).

Copied from yeastbridge/scripts/data/extract_esm2_embeddings.py (W1);
changes: --device auto (prefer GPU with >=8GB free, else CPU), config-driven
default paths (configs/product_transfer.json), and the torch>=2.6
safe-globals allowlist for the esm checkpoint's argparse.Namespace payload.
Batch budgeting logic unchanged.
"""
...
# esm2 ckpt 携带 argparse.Namespace;torch>=2.6 起 weights_only 默认为 True,需白名单
torch.serialization.add_safe_globals([argparse.Namespace])

_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_CFG = json.load(open(os.path.join(_ROOT, "configs/product_transfer.json")))
PROJECT = _CFG["yeastbridge_root"]
